...
def rule_search(d, signal = None, nbins = 100, nthresholds=10, longest_wait_time = 30000):
    if signal is None:
        ts = d['t/100ms']
        midprice = d['midprice/100ms']
        #bids = d['bid/100ms']
        #offers = d['offer/100ms']
        signal = analysis.time_til_movement(ts, midprice, prct = 0.0001, max_search=10000)
        #up, down = analysis.time_til_aggressive_profit(ts, bids, offers, prct = 0.0001, max_search = 60000, plot=False)
    # TODO: Also use down 
    
    inf_wait = signal > longest_wait_time  
    log_wait_times = np.log10(signal) 
